...
if(isset($_REQUEST["ACTION"]) && $_REQUEST["ACTION"]=="EDIT")
{
	...
	//$data = array();
	//$data["State_Name"]= $_REQUEST["State_Name"];
	
	//$data["FileCode"]	 	= $_REQUEST["ContactPerson_Mobile"];
	//$submitId = $objBaseLib->insert_query('state', $data, 'ID') ;  		
	//header("Location:state_entry.php?mode=". mysql_insert_id() ."&type=".$_REQUEST["submit"]);
}

if(isset($_GET["Save"]) && $_GET["Save"]=="Submit")
{
	$data = array();
	$data["State_Name"]= $_REQUEST["State_Name"];
	...
	<form name="frmApplication" method="post" onSubmit="return validate()">
		<table border="1" width="319">
			<tr>
				...
				<font color="#FFFFFF" face="Verdana" size="2"><b>State Master</b></font></td>
			</tr>
			<tr>
			...
			<tr>
				<td align="right"><font face="Verdana" size="2">State Name : </font></td>
				<td><font face="Verdana">
				<input type="text" name="State_Name" size="25" value="<?php echo $state; ?>"></font></td>
			</tr>
			<tr>
				<td align="right">&nbsp;</td>
				<td>&nbsp;</td>
		...
		<table border="1" width="100%">
			...
			<tr style="font-family:Verdana, Arial, Helvetica, sans-serif; font-size:12px">
				...
				<td width="180"><?php echo  $row['State_Name'];  ?></td>
				<td width="23" style="text-align:center">
				<a href="?ACTION=EDIT&ID=<?php echo  $row['ID'];  ?>&TEXT=<?php echo  $row['State_Name'];  ?>">Edit</a>
				</td>
				<td width="23" style="text-align:center">
				<a href="master_update.php?ACTION=DEL&ID=<?php echo  $row['ID'];  ?>" onClick="return confirm('Do you want to delete this record.')">Del</a>
				</td>
			</tr>
			<?php
			$i++;
