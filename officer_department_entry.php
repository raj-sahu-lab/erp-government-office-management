...
{
	$data = array();
	$data["PersonID"]	 	= $_REQUEST["PersonID"];
	$data["Department"]		= $_REQUEST["Department"];
	//$data["FileCode"]	 	= $_REQUEST["ContactPerson_Mobile"];
	$submitId = $objBaseLib->insert_query('person_department', $data, 'ID') ;  		
	header("Location:officer_department_entry.php?mode=". mysql_insert_id() ."&type=".$_REQUEST["submit"]);
 exit;
}
?>
<html>
	...
	<form name="frmApplication" method="post" onSubmit="return validate()">
		<table border="1" width="319">
			<tr>
				...
				Officer Department Master</b></font></td>
			</tr>
			<tr>
			...
			<tr>
				...
				<select name="PersonID" style="width:185px" >
                                <option value="0">Category</option>
                                <?php
                                $sql_officer =	" select ID, PersonName  from  person_master ";
                   
                                mysql_query("SET NAMES utf8");
                                $rs_officer = mysql_query($sql_officer);
                                while($row = mysql_fetch_array($rs_officer))
                                {
                                echo "<option value='". $row['ID'] ."' " . $pSelected . " >" . $row['PersonName'] ."</option>";
                                }
                                ?>
                </select>
			...
			<tr>
				<td align="right"><font face="Verdana" size="2">Department :</font></td>
		...
		<table border="1" width="100%">
			...
			<?php
			$sql_person =	" select ID, PersonName, Post, Person_Mobile, Contact_Person, ContactPerson_Mobile, FileCode from  person_master ";
			mysql_query("SET NAMES utf8");
			...
			<?php
			$sql_dpt =	" select Department from person_department where PersonID=". $row['ID'];
			mysql_query("SET NAMES utf8");
